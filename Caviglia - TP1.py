...
def RK4_quimica(x_0, y_0, a, b, h, N):
    ...
    while t[-1] <= N*h:  # Ejecuta el ciclo N veces
...
def ejercicio_3_a():
    ...
    # La cantidad de pasos N se ajusta de forma que h*N sea mantenga constate e igual a 100.
    
    plt.figure(dpi=300)
    
    x, y, _ = RK4_quimica(x_0, y_0, a, b, np.float64(0.1), 1000)
    plt.plot(x, y, markersize = 0.5, marker = "o", color = 'b', label = r'$h=0.1$')

    x, y, _ = RK4_quimica(x_0, y_0, a, b, np.float64(0.2), 500)
    plt.plot(x, y, markersize = 1.0, marker = "o", color = 'r', label = r'$h=0.2$')

    x, y, _ = RK4_quimica(x_0, y_0, a, b, np.float64(0.5), 200)
    plt.plot(x, y, markersize = 1.5, marker = "o", color = 'g', label = r'$h=0.5$')

    x, y, _ = RK4_quimica(x_0, y_0, a, b, np.float64(1.0), 100)
    plt.plot(x, y, markersize = 2.0, marker = "o", color = 'm', label = r'$h=1.0$')
...
def ejercicio_3_b():
    ...
    for x_0, y_0, nombre in zip( [np.float64(3.0), np.float64(1), np.float64(3.0), np.float64(1.5), np.float64(1.0)],
                                 [np.float64(3.0), np.float64(1), np.float64(1.0), np.float64(3.0), np.float64(3.0)],
                                 ['3b-fuera-1', '3b-fuera-2', '3b-dentro', '3b-cerca-fijo', '3b-fijo']):
        
        plt.figure(figsize = (4.5, 4), dpi=300)

        # La cantidad de pasos N se ajusta de forma que h*N sea mantenga constate e igual a 150.
        ...
        #x, y, _ = RK4_quimica(x_0, y_0, a, b, np.float64(0.355), 500)
...
def RK4_adapatativo(x_0, y_0, a, b, h, e, factor):
    ...
    while t[-1] <= 10:  # Bucle temporal: hace uso de la función RK4_quimica
...
def ejercicio_4(name, e=0.00001, factor = 1.5):   # Lo estructuro como función para pasar distintos e.     
    ...
    plt.plot(x_1, y_1, '-o', markersize = 2.0, color = 'm', label = r'$(x,t)(0) = (2,2)$')
    plt.plot(x_2, y_2, '-o', markersize = 2.0, color = 'y', label = r'$(x,t)(0) = (1,1)$')
    ...
    plt.plot(t_0, x_0, '-o', markersize = 2.0, color = 'g', label = r'$x(t)$')
    plt.plot(t_0, y_0, '-o', markersize = 2.0, color = 'b', label = r'$y(t)$')    
...
RK4_quimica(x_0, y_0, a, b, h, 1000)
end_time = time.time()
tiempo_1 = end_time - start_time

start_time = time.time()    
RK4_adapatativo(x_0, y_0, a, b, h, e, f)
end_time = time.time()
tiempo_2 = end_time - start_time

print("Sin paso adaptativo demoró:", tiempo_1, "s, mientras que con paso adapatito demoró", tiempo_2, "s.")
print("Porcentaje:," 100*(tiempo_1-tiempo_2)/tiempo_2)
"""
